function [Emsi,psnr] = DeNoising_tt_mdl(Nmsi,par,Omsi)
% MSI denoising based on TT-MDL
%==========================================================================
sizeData   = size(Nmsi);
Emsi       = Nmsi;
...
for  iter = 1 : par.deNoisingIter
    ...
    fprintf('TT denoising of iter %f has been done          ', iter);
    fprintf('\n')
    sizePart  =  5100;
...
%         for j = 1:sizeInd(2)
        parfor j = 1:sizeInd(2) 
            ...
            deY = ttsvd_mdl(Y,[]); 
%             deY = ttsvd_mdl(Y, R);  % use the tuned/suggested ranks if need
            deY = vec2tens(full(deY),sizeY); % 
